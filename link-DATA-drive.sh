...
if [[ $(findmnt -M /mnt/DATA) ]]; then   ### Modify DATA to whatever you want to call the mounted folder.
	...
	sudo mount /dev/sde1 /mnt/DATA ######## Modify with the drive that you are trying to mount. Use lsblk and blkid to find the correct one.
...
if grep -Fxq "UUID=53c104c0-ecee-43b4-9697-d12f14ce5ba8 /mnt/DATA ext4 defaults,noatime 0 2" /etc/fstab; then
	echo "Already in fstab"
else
	sudo echo 'UUID=53c104c0-ecee-43b4-9697-d12f14ce5ba8 /mnt/DATA ext4 defaults,noatime 0 2' >> /etc/fstab
fi
echo "---------done---------"
